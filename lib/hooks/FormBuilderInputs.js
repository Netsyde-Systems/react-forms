...
function getInputProps(formDefinition, formData, formState, fieldName, onFormChange) {
    ...
    if (typeof (fieldDef === null || fieldDef === void 0 ? void 0 : fieldDef.isRequired) == 'boolean')
        ...
        fieldRequired = fieldDef.isRequired(fieldValue, fieldName, formData);
    var errors = [];
    var errorMessage = undefined;
    if (fieldRequired) {
        errors.push.apply(errors, requiredFieldValidator(fieldValue, fieldName, formData));
    }
    if (fieldDef === null || fieldDef === void 0 ? void 0 : fieldDef.validators) {
        if (typeof fieldDef.validators === 'function') {
            errors.push.apply(errors, (_a = fieldDef.validators) === null || _a === void 0 ? void 0 : _a.call(fieldDef, fieldValue, fieldName, formData));
        }
        else if (Array.isArray(fieldDef.validators)) {
            errors.push.apply(errors, fieldDef.validators.flatMap(function (err) { return err(fieldValue, fieldName, formData); }));
        }
        else {
            switch (typeof fieldValue) {
                case 'string':
                    if (fieldDef.validators.max)
                        errors.push.apply(errors, getMaxLengthValidator(fieldDef.validators.max)(fieldValue, fieldName, formData));
                    if (fieldDef.validators.min)
                        errors.push.apply(errors, getMinLengthValidator(fieldDef.validators.min)(fieldValue, fieldName, formData));
                    break;
                case 'number':
                    if (fieldDef.validators.max)
                        errors.push.apply(errors, getMaxValidator(fieldDef.validators.max)(fieldValue, fieldName, formData));
                    if (fieldDef.validators.min)
                        errors.push.apply(errors, getMinValidator(fieldDef.validators.min)(fieldValue, fieldName, formData));
                    break;
            }
        }
    }
