...
var DefaultFileInputTextResources = {
...
export function FileInput(props) {
    ...
    var id = props.id, disabled = props.disabled, readOnly = props.readOnly, required = props.required, multiple = props.multiple, textResources = props.textResources, _a = props.maxFileSizeInBytes, maxFileSizeInBytes = _a === void 0 ? DEFAULT_MAX_FILE_SIZE_IN_BYTES : _a;
    var filledTextResources = Object.assign({}, DefaultFileInputTextResources, textResources);
    ...
    var uploadSection = readOnly ? null : (_jsxs("section", __assign({ className: uploadClassName }, { children: [_jsx("p", { children: filledTextResources.dragAndDropOr }), _jsxs("button", __assign({ type: "button", onClick: handleUploadBtnClick, disabled: disabled || readOnly }, { children: [_jsx("i", { className: "fas fa-file-upload" }), _jsx("span", { children: multiple ? filledTextResources.uploadFiles : filledTextResources.uploadAFile })] })), _jsx("div", __assign({ className: className }, { children: _jsx("input", __assign({ ref: fileInputField, type: 'file', title: '', value: '', onChange: handleNewFileUpload, multiple: multiple }, { id: id, disabled: disabled, readOnly: readOnly, required: required })) }))] })));
    return (_jsxs("div", __assign({ className: className }, { children: [_jsx(InputLabel, __assign({}, props)), uploadSection, _jsx("article", __assign({ className: 'preview' }, { children: _jsx("section", __assign({ className: 'file-list' }, { children: Object.keys(fileLookup).map(function (fileName, index) {
                        var file = fileLookup[fileName];
                        var isImageFile = file.type.split("/")[0] === "image";
                        var deleteIcon = disabled || readOnly ? null :
                            _jsx("i", { className: "fas fa-trash-alt", onClick: function () { return removeFile(fileName); } });
