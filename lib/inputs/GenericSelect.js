...
export function GenericSelect(props) {
    ...
    var options = props.selectOptions.map(function (option, optionIndex) {
        return (_jsx("option", { value: props.valueToString(option.value), children: option.text }, optionIndex));
    });
    if (!props.disallowBlank)
        options.unshift(_jsx("option", { value: props.valueToString(undefined), children: props.placeholder || '' }, ''));
    return (_jsxs("div", { className: className, children: [_jsx(InputLabel, __assign({}, props)), _jsx("select", __assign({}, controlProps, { value: props.valueToString(props.value), onChange: handleChange, id: id, disabled: disabled, required: required, children: options })), _jsx(ErrorMessage, __assign({}, props))] }));
}
