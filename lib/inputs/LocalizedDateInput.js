...
export function LocalizedDateInput(props) {
    ...
    var id = props.id, disabled = props.disabled, min = props.min, max = props.max;
    return (_jsxs("div", __assign({ className: className }, { children: [_jsx(InputLabel, __assign({}, props)), _jsx(DatePicker, __assign({ onChange: handleChange, value: value, locale: props.locale, minDate: min, maxDate: max, showMonthDropdown: true, showYearDropdown: true, autoComplete: 'off' }, { id: id, disabled: disabled })), _jsx(ErrorMessage, __assign({}, props))] })));
}
