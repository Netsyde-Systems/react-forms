...
export function TextRadio(props) {
    var handleOptionChange = function (e) { return props.onChange(e.target.value); };
    ...
    var radios = props.selectOptions.map(function (option, optionIndex) {
        ...
        return (_jsxs("div", { children: [_jsx("input", { id: radioId, name: props.id, type: 'radio', value: option.value, checked: props.value == option.value, onChange: handleOptionChange, disabled: props.disabled }), _jsx("label", __assign({ htmlFor: props.id + optionIndex }, { children: option.text }))] }, optionIndex));
    });
